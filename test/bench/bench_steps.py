...
STEPS_TEMPO = [
 ...
 ('Knob pickup on the first touch',
  'PASS IF: Two readings are possible and both are correct -- the row tells you which. Held reads bpm 120 (10) with two numbers and the footer stays at 120-bpm. Released reads bpm 10 on its own and the footer goes to 10-bpm. Held means knob 1 was saved somewhere other than the bottom. Released means it was saved at the bottom or there is no knobs.txt at all. Either way the row must never read og-knob-1 and never a 0-to-1 decimal.',
  [('120', 'tempo'), ('og-knob-1 0', 'param')]),
 ('Knob pickup crosses over',
  'PASS IF: The footer reads 500-bpm. A slide up rather than a snap is correct -- the knob has crossed its stored position and is live from here on.',
  [('og-knob-1 1', 'param')]),
 ('Knob tracking after pickup',
  'PASS IF: The footer reads 10-bpm and tracks the knob straight down with no hold at all. If it is held here too then the knob is stuck and that is the failure.',
  [('og-knob-1 0', 'param')]),
 ('Tempo above range',
