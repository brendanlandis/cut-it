...
| **Phase 6 on the hardware** | Nothing — but Phase 6 is not done until it runs there | ✅ **DEPLOYED 2026-08-03, the first time.** Syntax check passed, 18 files copied, patch loaded, and with the Launchpad and nanoKONTROL on a USB hub `aconnect` shows the Launchpad wired **both ways** and the nano in. ✅ Item 94 measured — **over its stated budget at 11.7–12.0 %, but the budget is not a like-for-like ceiling** and what the item exists to prove, that the dirty flag gates, is proved. ⬜ **Still open: items 96, 97, 95** — the safe exit after the lift (costs a power cycle if wrong), the boot sequence in its real order, and full-rig power. And the stepped bench has never been run ON the device |
...
|---|---|
| **A replug leaves `m_launchpad` believing it still owns the surface** | ✅ **FIXED AND CONFIRMED ON HARDWARE 2026-08-04 — `m_launchpad` → `pd watchdog`.** A heartbeat re-asserts Programmer Mode every 2 s (the Mac cannot be fixed by polling — the device answers the inquiry in *either* mode) and an inquiry poll detects loss on the Organelle, where a replug destroys the ALSA subscriptions outright. Recovery re-runs `wire.sh`: 8 attempts, first at ~14 s, stopping at ~70 s. Unplugged and replugged on the device, the grid returned by itself **in the correct state** — mode lamp, beat row, pads reporting, nano still driving the lamp. See [plan-tests.md](plan-tests.md) item 112 for everything measured on the way, including the two flaws found — an arming gate the assert layer caught, and a 12-second give-up window that only hardware could show was useless |
...
| **The Launchpad's own bottom row, CC 1–8** | ✅ **FIXED — span widened from 10–108 to 1–108**, 108 specs, frame 305 → 332 bytes. The old reason for excluding it was measured false: "120 is rejected outright" came from three broken probes, not a ceiling (items 105, 109). **The real cost was that an index outside the span can never be cleared** — LED state survives the Programmer Mode switch, so whatever Live Mode drew there persisted into every session. Confirmed twice over during the Phase 6 run: the row was green one pass and dark the next with nothing in Cut It touching it. `tools/phase6-assert.py` updated to 332 bytes and `9 + 3*(i-1)`; 29 checks pass |
| **Index 0, the Launchpad's Setup button** | ✅ **ANSWERED — it is not ours, and the span stops at 1.** Item 110: a valid one-spec frame addressing index 0 with colour 21 (`PAINT-SENT-BYTES: 11` = 7 header + 3 + terminator, so the send was well formed) lit **nothing**, and pressing Setup put **nothing** on `RAW-IN`. So it takes no colour and transmits no MIDI in Programmer Mode. `$0-surface` still allocates cell 0 and deliberately never writes it. This also sidesteps an unknown worth noting: had index 0 been included in a larger frame and the device rejected the whole message, every repaint would have broken |
| **MIDI clock cost scales with tempo** | ✅ **MEASURED 2026-08-03 — MUCH GENTLER THAN PREDICTED, and it contradicts what we thought we knew.** At 120 BPM the clock is 48 pulses/s on two ports (~96 ALSA writes/s); at the 500 BPM top of knob 1's range it is ~400. **CPU went 10.6–11.3 % → 11.7–11.9 %: one point for 4.17× the rate.** So tempo scaling is real but cheap, and no action is needed. ✅ **ITEM 75 IS NOW SETTLED BY DIRECT ISOLATION: DSP off reads 4.9 % against 11.8 % on, so the DSP costs 6.9 points and the MIDI 0.43.** The note below predicted this from the slope and it held. ⚠️ **Consequence: the port-1 idea below is no longer worth doing for CPU** — it would save about 0.2 points. Keep it only if something else wants the Launchpad's clock silenced. The original point:, which blamed the Phase 4 → 5 doubling (5.3 → 10.2 %) on those 96 writes and marked it ⬜ *not confirmed by isolation*. If 96 writes cost ~5 points, 304 more should cost ~15. They cost **1**. **The doubling was almost certainly the DSP that arrived in the same phase, not the MIDI** — worth isolating before v0.3 stacks four filter stages on that baseline. ⬜ **The cheap mitigation is still available if ever needed**: stop sending clock to port 1. `u_tempo` sends it there so the Launchpad can pace *flash and pulse* animations, and Cut It uses neither — `g_grid` emits only static specs, which `tools/phase6-assert.py` asserts outright. That halves the writes at no cost to anything in use. Port 3 must stay: the SP-404 learns tempo only by measuring pulse intervals |
| **`g_grid`'s frame clock and the beat-row swing** | ✅ **FIXED AND MEASURED — `[metro 100]` → `[metro 20]`.** At 10 Hz the row could only move on a 100 ms boundary: at 120 BPM a beat is 500 ms and divides exactly, but at 240 it is 250 ms and does not, so beats landed alternately at 200 and 300 ms — a **±50 ms alternating swing**, 20 % of the beat interval and plainly visible. Seen on the Phase 6 Mac run. At 50 Hz the worst case is ±10 ms. ✅ **The CPU hedge was wrong and is now measured**: 100 → 20 read **11.5 / 11.6 / 12.0 %** against **11.7 / 11.9 / 12.0 %**, indistinguishable. The metro checks the dirty flag, it does not repaint, so the frame count is bounded by the beat rate and never by the metro. Item 94 |
