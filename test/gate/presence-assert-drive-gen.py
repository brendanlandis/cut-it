...
THE SCHEDULE IS u_present's OWN ARITHMETIC, and none of it is free:
    ...
    8000 ms   c_presence's third missed tick. m_launchpad is declared lost, and
              u_present's recovery counter starts in the SAME tick, because the
              tick is broadcast before the recovery step reads the gate
   14000 ms   counter 4 -- mod 4 hits 0 and wire.sh is forked FOR THE FIRST TIME
   22000 ms   counter 8 -- the second fork
...
SEQ = [
    ...
    # A third re-wire would have been due at 29000. The device answered, so the
    # miss counter zeroed, moses went left and the recovery counter was reset --
    # nothing should fork here.
    (28000, "RECOVERED", [], GAP),
]
QUIT_MS = 32000
