...
Twenty-one gates, and each answers for exactly one page under `ref/`. That is the whole organising
...
| `paper` | no Pd at all | **`file` predicates only** — the evidence is on disk, and `debug` has none at all because its output is a screen. Auto-selected for `state`, `recover` and `debug` |
...
**All nine benches are generated by `bench-gen.py` from the step tables in `bench_steps.py`**
(`STEPS_DISPLAY` through `STEPS_MIDI`). Edit the table and re-run the generator; **never edit a
`phaseN-bench.pd`.**

```sh
python3 test/bench/bench-gen.py        # writes all eight, plus bench-tap.pd
...
A driven bench loads as a **third patch** after `mother.pd` and `main.pd`, which is what
gives it a real console.

⛔ **And that is exactly why `recover` is a PAPER bench.** Its third step holds CC 90, which reloads
the patch — `/loadPatch` kills that Pd process, and the bench is *inside* it, so a driven run would
end there with every later step stranded. With no actions, no `reload` and only offline predicates
the runner launches nothing at all: a person runs those steps against the real deployed instrument,
and the one thing under test cannot take the runner down with it. `state` is paper for a quieter
reason — its subject is files.

This is the launch line for the driven ones:
...
**Seven steps, and the only bench that deliberately breaks the instrument.** Most of what `recover`
does is proven headlessly — `recover-assert.sh` lints the two-step OSC by reading it and drives both
tiers against a stubbed `[shell]`. **What no gate can do is load a patch**, so the reload itself
lives here and only here.

⛔ **It is PAPER**, for the reason in *Running a bench on the device* above: step 3 holds CC 90, the
patch reloads, and a driven bench would die with it.

| | |
|---|---|
| 1–2 | The **short press**. Everything stops on the press rather than the release, the grid flashes red and clears itself, and the Volca's sequencer stops too — the last of which never worked before item 295 |
| 3 | The **hold**. One verdict covering the reload, the re-enumeration, the late `recovered` alert and the knobs being live at once — because after step 3 there is no bench left to ask a second question |
| 4 | The breadcrumb was **consumed** — `none` means the whole cycle completed |
| 5–6 | ⛔ **The reload is made to fail on purpose**, by renaming the patch folder. Nothing comes back, and the breadcrumb is then the *only* account of it: a patch cannot report on its own reload because it is dead by then |
| 7 | Puts the rig back, and checks the **safe exit** on the way out |

⚠️ **Step 2 needs the Volca's `MIDI Clock src` set to `Auto`.** Set to `Internal` it ignores Start
and Stop entirely and a correct patch looks broken. That is Korg's documentation rather than
something measured here, which is why it is a `need`.

⚠️ **Step 5 leaves the rig with no patch at all.** Step 6 reads the file and step 7 puts it back,
and both say so in their own `need` — a step sets up its own preconditions, including the ones the
step before it destroyed.

### `debug-bench.pd` — the debug patch acceptance run

**Nine steps, and the only bench with no predicate at all.** That is honest rather than lazy:
[the debug patch](../ref/module/debug.md) writes nothing to disk because its output **is** the OLED,
which is the whole point of a tool for a venue with no laptop. Inventing a logfile so a predicate had
something to read would be a test hook wearing a feature's clothes.

⛔ **It is PAPER**, and for the same reason `recover` is: step 2 **loads another patch**, which is a
Pd restart, and a driven bench runs as a third patch inside the instrument's own Pd.

⛔ **Step 3 is the one everything else rests on.** Move a nanoKONTROL fader and watch the `nano` row
count. Loading any patch drops Pd's ALSA connections and the patch runs `wire.sh` itself to put them
back — so a row stuck on zero means the tool is measuring silence, and **nothing any later step
reports about any other device means anything until that row moves.** The nano is the right prover:
it transmits the moment you touch it and has nothing to go wrong at the far end.

| | |
|---|---|
| 1–2 | It is in `! debug` and not beside the instrument, and it comes up on `1-MIDI-IN` |
| 3 | ⛔ **It wired itself.** Prove the probe before believing the silence |
| 4 | Each device lands on its **own row** — the fault this screen exists to find is two devices sharing one, which is mother's auto-connect winning (item 274) |
| 5–6 | The error log tail, and **the address you type into the phone** (item 312) |
| 7 | Both output devices answer, one probe per press. ⚠️ Pad A1 may start a **looping** sample only the device can stop (item 311) |
| 8 | A re-wire by hand reports its link count — ⚠️ the one row no gate can reach, because `t_shell` emits nothing to parse |
| 9 | ⛔ **It puts the rig back.** Everything above ran with the instrument stopped |

⚠️ **Every step runs with Cut It not running.** That is what selecting any patch means on this
device, and it is why this bench is not one to slot into the middle of a rig session.
