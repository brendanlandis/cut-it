...
# hardware, ~25 s.
#
#   ./tools/phone-assert.sh          run it, exit non-zero on any failure
#   ./tools/phone-assert.sh -v       and dump every decoded datagram
#
...
python3 tools/phone-assert-drive-gen.py
exec python3 tools/phone-assert.py "$@"
