...
STEPS_TEMPO = [
 ...
 ('TEMPO THROUGH THE MAP -- og-knob-1 at 0 -- PICKUP HOLDS IT',
  "PASS IF: ⛔ THIS ONE DIFFERS BY MACHINE AND THE DIFFERENCE IS PICKUP WORKING. ON THE DEVICE the footer does NOT move -- mother restored knob 1 at boot and 0 is below that stored value so the knob is HELD until it crosses back. ON THE MAC nothing pushes knobs.txt at all so this is knob 1's first value ever -- it lands and the footer reads 10-bpm with the 404's EXT sliding down",
  [('og-knob-1 0', 'param')]),
 ('TEMPO THROUGH THE MAP -- og-knob-1 at 1 -- THE CROSSING',
  "PASS IF: the footer reads 500-bpm and the 404's EXT slides up. ON THE DEVICE this is the moment the knob crosses the restored value and TAKES OVER -- held one step ago and live now. A slide rather than a snap IS the inference working -- it snaps only to a tempo it has already learned",
  [('og-knob-1 1', 'param')]),
