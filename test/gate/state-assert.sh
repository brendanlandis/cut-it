...
# no socket, ~12 s.
#
#   ./test/gate/state-assert.sh          run it, exit non-zero on any failure
#   ./test/gate/state-assert.sh -v       and show the detail behind every check
#
...
python3 test/gate/state-assert-drive-gen.py test/gate/state-assert-drive.pd /tmp/cut-it-state-gate
exec python3 test/gate/state-assert.py "$@"
