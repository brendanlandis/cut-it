...
#
#   ./tools/phase9-assert.sh          run it, exit non-zero on any failure
#   ./tools/phase9-assert.sh -v       and show the detail behind every check
...
cp tools/test-stubs/t_noteout.pd tools/test-stubs/t_ctlout.pd \
   tools/test-stubs/t_pgmout.pd tools/test-stubs/t_notein.pd "$WORK/patch/"
...
if ! python3 tools/phase9-assert-drive-gen.py "$WORK/drive.pd" >/dev/null; then
...
python3 tools/phase9-assert.py $ARGS < "$CAP"
rc=$?
