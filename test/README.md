...
**609 checks.** ⚠️ **Nineteen of the twenty gates print their own `N checks` line and one does
not** — `midi-emitters-assert.sh` prints an inventory instead, so its 7 is hand-maintained and the
total cannot be derived from a run by summing. Totalling the run gives **602**; the difference is
...
**All eight benches are generated by `bench-gen.py` from the step tables in `bench_steps.py`**
